...
class LakeLevel(BaseModel):
    ...
    head_elevation: float | None = Field(
        None, description="Head elevation above dam (feet)"
    )
    tail_elevation: float | None = Field(
...
class RiverCondition(BaseModel):
    ...
    bankfull_stage: float | None = Field(
...
class RiverForecast(BaseModel):
    ...
    issued_by: DataSource | None = Field(
...
class FloodgateOperation(BaseModel):
    ...
    lake_level_forecast: str | None = Field(
        None, description="Lake level forecast description"
    )
    current_elevation: float | None = Field(
